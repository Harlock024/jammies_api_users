...
public class PlaylistService {
    ...
    public PlaylistService(PlaylistsRepository playlistsRepository, CloudinaryService cloudinaryService, TrackRepository trackRepository, TrackServices trackServices, ContentNegotiatingViewResolver contentNegotiatingViewResolver) {
    ...
    public List<PlaylistResponseDto> getAllPlaylists() {
        return playlistsRepository.findAll().stream().map(this::convertToDto).collect(Collectors.toList());
    }

    public PlaylistResponseDto getPlaylist(UUID playlistId) {
        return playlistsRepository.findById(playlistId).map(this::convertToDto).orElseThrow(() -> new NoSuchElementException("Playlist not found"));
    }
    ...
    public PlaylistResponseDto updatePlaylist(UUID id, PlaylistRequestDto playlistRequestDto) throws IOException {

        Playlist playlist = playlistsRepository.findById(id).orElseThrow(() -> new NoSuchElementException("Playlist not found"));
        ...
        if (playlistRequestDto.getCover() != null) {
            var coverResult = cloudinaryService.upload(playlistRequestDto.getCover(), "jammies_track/playlist/cover", "image");
            if (coverResult == null) {
    ...
    public PlaylistResponseDto convertToDto(@NotNull Playlist playlist) {
        return new PlaylistResponseDto(playlist.getId(), playlist.getName(), playlist.getDescription(), playlist.getUser() != null ? playlist.getUser().getUsername() : null, playlist.getCover_url());
    }
    // track to playlist


    public PlaylistResponseDto AddTrackToPlaylist(UUID playlistId, UUID trackId) {
        Playlist playlist = playlistsRepository.findById(playlistId).orElseThrow(() -> new NoSuchElementException("Playlist not found"));
